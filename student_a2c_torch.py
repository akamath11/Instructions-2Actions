...
class StudentA2C(A2C):
    ...
    def student_loss(self, target, pred_prob, g, teacher_actor_prob, r_teacher):
        ...
        for i in range(len(teacher_actor_prob)):
            ...
            cel = torch.mean(self.ce_loss(pred_prob, targets))
            print("\nA2C Loss: {},  KL Loss : {}".format(a2c_loss.data.numpy().flatten()[0], self.epsilon * cel.data.numpy()))
        return a2c_loss + self.epsilon * cel
